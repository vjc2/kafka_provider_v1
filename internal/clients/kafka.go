...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		kafkaCreds := map[string]string{}
		if err := json.Unmarshal(data, &kafkaCreds); err != nil {
		...
		/*ps.Env = []string{
			fmt.Sprintf("%s=%s", "HASHICUPS_USERNAME", kafkaCreds["username"]),
			fmt.Sprintf("%s=%s", "HASHICUPS_PASSWORD", kafkaCreds["password"]),
		}*/
		// set credentials in Terraform provider configuration
		/*ps.Configuration = map[string]interface{}{
			"username": kafkaCreds["username"],
			"password": kafkaCreds["password"],
		}*/
		return ps, nil
	}
}
